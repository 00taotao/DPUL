...
def FedAvg_noniid_float(w, client_data_sizes):
    ...
    return w_avg
def FedAvg_noniid_float_www(w,w1,w2,client_data_sizes,idx):
    """
    Args:
    w (list): List of client weights.
    client_data_sizes (list): List of data sizes for each client.
    ...
    """
    total_data = float(sum(client_data_sizes))  # 计算总的数据量
    num_size = client_data_sizes[idx]
    ...
    for k in w_avg.keys():
        w_avg[k] += w[k].float() + (w1[k].float() - w2[k].float()) * (float(num_size)/ total_data)
